...
# )
    ...
    # allow_origins=["http://localhost:3000", "http://10.144.119.175:3000"],  # Update with your Next.js application's origin
...
async def upload_image(request: Request, image_data: UploadFile = File(...)):
    ...
    if image is not None:
        ...
        # predict(model, image, classes)
        # process_image = identify_leaf(file_path)
        
        process_image = identify_leaf(image)
    ...
    print('THE VERIFY IMAGE FUNCTION RESPONSE ===> ',process_image)
...
# uvicorn.run(app, host=["localhost", "10.144.119.175"], port=8000)
...
# uvicorn app:app --host 10.144.119.175 --port 8000  # Binds to all interfaces
